...
export default class SearchableDropDown extends Component {
      ...
      setSort = (item, searchedText) => {
        return (
          item.businessAddress
            .toLowerCase()
            .indexOf(searchedText.toLowerCase()) > -1
        );
      };
    }
  ...
  renderTextInput = () => {
    ...
    const oldSupport = [
      ...
      {
        key: 'value',
        val: this.state.item ? this.state.item.businessAddress : '',
      },
      {
        key: 'style',
          ...
          {items.map((item, index) => {
            return (
              <View
                key={index}
                style={{
                  width: item.businessAddress.length * 8 + 60,
                  justifyContent: 'center',
                  flex: 0,
                  backgroundColor: '#eee',
                  flexDirection: 'row',
                ...
                <Text style={{color: '#555'}}>{item.businessAddress}</Text>
                <TouchableOpacity
                  onPress={() =>
                    setTimeout(() => {
                      this.props.onRemoveItem(item, index);
